...
def extract_8_features(url, domain_age=30):
    try:
        parsed = urlparse(url)
        hostname = parsed.hostname if parsed.hostname else ""
        
        # Feature 8: The Popularity Check
        # Check if the root domain (e.g., google.com) is in our top 10k set
        is_popular = 1 if hostname in top_10k or any(hostname.endswith('.' + d) for d in top_10k) else 0
...
def generate_synthetic_data(n_samples=20000):
    ...
    for _ in range(n_samples // 2):
        ...
        feature_vector = extract_8_features(url, random.randint(500, 3000))
    ...
    for _ in range(n_samples // 2):
        ...
        feature_vector = extract_8_features(url, random.randint(0, 30))
...
initial_type = [('float_input', FloatTensorType([None, 8]))] # Note the 8 here!
...
print("Done! 'phishing_prober.onnx' is ready with 8 features.")
